class Ship:
    ...
    def set_size(self, size, checklist_ships, count_ship):
        """
        Метод проверки размера корабля. Елси размер не подходит ни к одному условию, то возращается False.
        Если размер подходит, то атрибуту объекта присваивается новый размер, и уменьшается счетчик кораблей.
